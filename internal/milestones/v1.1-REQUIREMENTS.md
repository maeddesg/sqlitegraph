# Requirements Archive: v1.1 ACID & Reliability

**Archived:** 2026-01-20
**Status:** ✅ SHIPPED

This is the archived requirements specification for v1.1 ACID & Reliability.
...
- [x] **ACID-01**: Node deletion captures before-image data (node record, all edges)
- [x] **ACID-02**: Node deletion rollback restores node to previous state
- [x] **ACID-03**: Node deletion rollback reclaims allocated slots
- [x] **ACID-04**: Node deletion rollback restores all incoming and outgoing edges
- [x] **ACID-05**: WAL recovery correctly handles IN_PROGRESS transactions (treat as ABORTED)
- [x] **ACID-06**: Rollback operations are crash-safe (rollback state persisted to WAL)
...
- [x] **ACID-19**: Transaction-count checkpoint trigger fires after N transactions
- [x] **ACID-20**: Size-based checkpoint trigger fires when WAL exceeds threshold
- [x] **ACID-21**: WAL manager tracks transaction count since last checkpoint
- [x] **ACID-22**: WAL manager tracks current WAL file size
- [x] **ACID-23**: All three checkpoint strategies are tested and functional
...
- [ ] **HNSW-10**: HNSW persistence stores layer information per vector (DEFERRED to v1.2)
...
- [x] **CP-01**: Transaction-count checkpoint returns true when threshold exceeded
- [x] **CP-02**: Size-based checkpoint returns true when WAL size exceeds threshold
- [x] **CP-03**: Checkpoint triggers are configurable via NativeConfig
- [x] **CP-04**: Checkpoint triggers reset their counters after checkpoint completes
...
- [x] **REFAC-05**: validator.rs (11,000 LOC) split into modules (header, cluster, wal)
...
- [x] **CW-01**: Concurrent write design document defines architecture
- [x] **CW-02**: Lock acquisition ordering prevents deadlocks
- [x] **CW-03**: Multiple writers can commit transactions concurrently
...
- [x] **TEST-WAL-01**: Node deletion rollback test passes
...
- Shipped: 77/78 (98.7%)
- Deferred: 1 (HNSW-10 - layer persistence, explicitly marked for v1.2)

---

## Milestone Summary

**Shipped:** 77 of 78 v1.1 requirements (98.7%)
**Deferred:** 1 (HNSW-10 - requires database schema migration)
**Adjusted:** None
**Dropped:** None
