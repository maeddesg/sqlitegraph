...
mod tests {
    ...
    fn test_natural_loop_nesting() {
        // Scenario: Test is_nested_in() method
        // Expected: Inner loop nested in outer
        let graph = create_nested_loops_cfg();
        let entity_ids: Vec<i64> = graph.list_entity_ids().expect("Failed to get IDs");
        let entry = entity_ids[0];
        ...
        // Outer is not nested in inner
        assert!(!outer.is_nested_in(&inner), "Outer should not be nested in inner");
    }
    ...
    fn test_natural_loops_nesting_depth() {
        ...
        // Expected: nesting_depth() returns correct depth for each node
        let graph = create_nested_loops_cfg();
        let entity_ids: Vec<i64> = graph.list_entity_ids().expect("Failed to get IDs");
        let entry = entity_ids[0];
        ...
        // Outer header is in 1 loop (itself)
        assert_eq!(loops.nesting_depth(entity_ids[1]), 1, "Outer header should have depth 1");

        // Inner header is in 2 loops (outer + inner)
        assert_eq!(loops.nesting_depth(entity_ids[2]), 2, "Inner header should have depth 2");

        // Node 3 is in both loops
        assert_eq!(loops.nesting_depth(entity_ids[3]), 2, "Node 3 should have depth 2");
    }
    ...
    fn test_natural_loops_loops_at_depth() {
        ...
        // Expected: loops_at_depth() filters correctly
        let graph = create_nested_loops_cfg();
        let entity_ids: Vec<i64> = graph.list_entity_ids().expect("Failed to get IDs");
        let entry = entity_ids[0];
        ...
        assert_eq!(depth1.len(), 1, "Should have 1 loop at depth 1");
        assert_eq!(depth1[0].header, entity_ids[1], "Depth 1 should be outer loop");

        // Depth 2 = inner loops
        let depth2 = loops.loops_at_depth(2);
        assert_eq!(depth2.len(), 1, "Should have 1 loop at depth 2");
        assert_eq!(depth2[0].header, entity_ids[2], "Depth 2 should be inner loop");
    }
