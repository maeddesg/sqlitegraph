...
mod tests {
    ...
    fn test_transitive_reduction_linear_chain() {
        ...
        assert!(
            essential.contains(&(entity_ids[0], entity_ids[1])),
            "Edge 0->1 should be essential"
        );
        assert!(
            essential.contains(&(entity_ids[1], entity_ids[2])),
            "Edge 1->2 should be essential"
        );
        assert!(
            essential.contains(&(entity_ids[2], entity_ids[3])),
            "Edge 2->3 should be essential"
        );
    }
    ...
    fn test_transitive_reduction_diamond() {
        ...
        assert_eq!(essential.len(), 4, "Should have 4 essential edges after reduction");

        // Verify essential edges are present
        assert!(
            essential.contains(&(entity_ids[0], entity_ids[1])),
            "Edge 0->1 should be essential"
        );
        assert!(
            essential.contains(&(entity_ids[0], entity_ids[2])),
            "Edge 0->2 should be essential"
        );
        assert!(
            essential.contains(&(entity_ids[1], entity_ids[3])),
            "Edge 1->3 should be essential"
        );
        assert!(
            essential.contains(&(entity_ids[2], entity_ids[3])),
            "Edge 2->3 should be essential"
        ...
        assert!(
            !essential.contains(&(entity_ids[0], entity_ids[3])),
            "Edge 0->3 should be removed (redundant)"
        );
    }
    ...
    fn test_transitive_reduction_fully_connected() {
        ...
        // Total edges: 4*3/2 = 6
        // Essential edges: 3 (the "covering" edges: 0->1, 1->2, 2->3)
        // Redundant edges: 3 (0->2, 0->3, 1->3)
        assert_eq!(essential.len(), 3, "Should have 3 essential edges");

        // Verify only the minimal edges remain
        assert!(
            essential.contains(&(entity_ids[0], entity_ids[1])),
            "Edge 0->1 should be essential"
        );
        ...
        // Verify long edges are removed
        assert!(
            !essential.contains(&(entity_ids[0], entity_ids[2])),
            "Edge 0->2 should be removed (redundant via 0->1->2)"
        );
        assert!(
            !essential.contains(&(entity_ids[0], entity_ids[3])),
            "Edge 0->3 should be removed (redundant via chain)"
        );
        assert!(
            !essential.contains(&(entity_ids[1], entity_ids[3])),
            "Edge 1->3 should be removed (redundant via 1->2->3)"
        );
    }
    ...
    fn test_transitive_reduction_with_progress() {
        ...
        let essential = result.unwrap();
        assert!(essential.len() > 0, "Should have essential edges");
    }
