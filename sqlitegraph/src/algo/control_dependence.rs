...
mod tests {
    ...
    fn test_control_dependence_if_then_else() {
        ...
        // Expected: Node 3 (merge point) is control-dependent on node 0 (branch)
        ...
        // Node 3 should be control-dependent on node 0
        assert!(cdg_result.controls(entity_ids[0], entity_ids[3]),
                "Node 0 should control node 3");
        assert!(cdg_result.depends_on(entity_ids[3], entity_ids[0]),
                "Node 3 should depend on node 0");

        // Verify in forward mapping
        assert!(cdg_result.cdg.get(&entity_ids[0])
                .map(|set| set.contains(&entity_ids[3]))
                .unwrap_or(false));

        // Verify in reverse mapping
        assert!(cdg_result.reverse_cdg.get(&entity_ids[3])
                .map(|set| set.contains(&entity_ids[0]))
                .unwrap_or(false));
    }
    ...
    fn test_control_dependence_nested_if() {
        ...
        // Expected: Nested dependencies (3 and 4 depend on 1, which depends on 0)
        ...
        // Node 5 should depend on node 0 (merge of outer if)
        assert!(cdg_result.depends_on(entity_ids[5], entity_ids[0]),
                "Node 5 should depend on node 0");

        // Node 5 should depend on node 1 (merge of inner if)
        assert!(cdg_result.depends_on(entity_ids[5], entity_ids[1]),
                "Node 5 should depend on node 1");

        // Nodes 3 and 4 should depend on node 1 (inner if branches)
        assert!(cdg_result.depends_on(entity_ids[3], entity_ids[1]),
                "Node 3 should depend on node 1");
        assert!(cdg_result.depends_on(entity_ids[4], entity_ids[1]),
                "Node 4 should depend on node 1");
    }
    ...
    fn test_control_dependence_linear_chain() {
        ...
        // CDG should be empty (no control dependence in straight-line code)
        assert_eq!(cdg_result.cdg.len(), 0,
                   "CDG should be empty for linear chain");
        assert_eq!(cdg_result.reverse_cdg.len(), 0,
                   "Reverse CDG should be empty for linear chain");

        // No node should control any other node
        for &node in &entity_ids {
            assert_eq!(cdg_result.controlled_by(node), None);
            assert_eq!(cdg_result.dependencies_of(node), None);
        }
    }
    ...
    fn test_reverse_cdg_lookup() {
        ...
        // Node 3 should have dependencies
        let deps = cdg_result.dependencies_of(entity_ids[3]);
        assert!(deps.is_some(), "Node 3 should have dependencies");
        assert!(
            deps.map(|set| set.contains(&entity_ids[0])).unwrap_or(false),
            "Node 3 should depend on node 0"
        );
    ...
    fn test_reverse_cdg_empty_for_no_dependencies() {
        // Scenario: Nodes with no control dependence have empty sets
        // Expected: dependencies_of returns None, not Some(empty_set)
        let graph = create_linear_chain();
        let entity_ids: Vec<i64> = graph.list_entity_ids().expect("Failed to get IDs");
        let exit = entity_ids[3];
        ...
        // All nodes should have None (not Some(empty))
        for &node in &entity_ids {
            assert_eq!(cdg_result.dependencies_of(node), None,
                      "Node {} should have None dependencies, not Some(empty)", node);
            assert_eq!(cdg_result.controlled_by(node), None,
                      "Node {} should have None controlled, not Some(empty)", node);
        }
    }
    ...
    fn test_control_dependence_single_edge() {
        ...
        // No control dependence (linear edge is fall-through)
        assert_eq!(cdg_result.cdg.len(), 0);
        assert_eq!(cdg_result.reverse_cdg.len(), 0);
    }
    ...
    fn test_control_dependence_diamond_no_merge_dep() {
        ...
        // Expected: Node 3 has control dependence on node 0
        let graph = create_if_then_else_cfg();
        let entity_ids: Vec<i64> = graph.list_entity_ids().expect("Failed to get IDs");
        let exit = entity_ids[3];
        ...
        // Node 3 (merge) should be control-dependent on node 0 (branch)
        assert!(cdg_result.controls(entity_ids[0], entity_ids[3]));
        assert!(cdg_result.depends_on(entity_ids[3], entity_ids[0]));
    }
    ...
    fn test_control_dependence_multiple_branches() {
        ...
        // Expected: Node 4 (merge) is control-dependent on node 0 (branch)
        let graph = SqliteGraph::open_in_memory().expect("Failed to create graph");
        ...
        // Node 4 (merge) should be control-dependent on node 0 (branch)
        assert!(cdg_result.controls(entity_ids[0], entity_ids[4]));
        assert!(cdg_result.depends_on(entity_ids[4], entity_ids[0]));
    }
    ...
    fn test_control_dependence_from_post_result() {
        ...
        // Should have control dependence
        assert!(cdg_result.controls(entity_ids[0], entity_ids[3]));
    }
    ...
    fn test_control_dependence_controls_method() {
        ...
        assert!(cdg_result.controls(entity_ids[0], entity_ids[3]));
        assert!(!cdg_result.controls(entity_ids[3], entity_ids[0])); // Not symmetric
    }
    ...
    fn test_control_dependence_depends_on_method() {
        ...
        assert!(cdg_result.depends_on(entity_ids[3], entity_ids[0]));
        assert!(!cdg_result.depends_on(entity_ids[0], entity_ids[3])); // Not symmetric
    }
}
