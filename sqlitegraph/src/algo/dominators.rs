...
fn reverse_postorder(
    ...
    while let Some((node, visited_children)) = stack.pop() {
        ...
        if visited_children {
            ...
            // First time seeing this node, push it back to add after children
...
fn extract_immediate_dominators(
    ...
    for (&node, dom_set) in dom {
        ...
        // Find immediate dominator: the strict dominator that is NOT dominated
        // by any other strict dominator (i.e., the closest one)
        ...
        for &candidate in &strict_dominators {
            let _is_closest = strict_dominators
                ...
                .all(|&other| candidate == other || !dom_set.contains(&other) || {
                    ...
                    // If 'other' is in candidate's dominance set, then other dominates candidate
                    // So candidate is NOT the closest if another strict dominator dominates it
                    if let Some(candidate_dom) = dom.get(&candidate) {
                        !candidate_dom.contains(&other)
                    } else {
                        false
                    }
                });

            // Simpler check: immediate dominator is the strict dominator that
            // does NOT have any other strict dominator in its own dominance set
            // (except itself and the node)
            if let Some(candidate_dom) = dom.get(&candidate) {
                let has_other_strict_dominator = strict_dominators
                    .iter()
                    .any(|&other| other != candidate && other != node && candidate_dom.contains(&other));

                if !has_other_strict_dominator {
...
mod tests {
    ...
    fn test_dominators_diamond() {
        ...
        // Expected: 0 dominates all, 1 dominates {1,3}, 2 dominates {2,3}
        ...
        // Node 1 dominates {1, 3} (not 2, which is parallel branch)
        assert!(result.dominates(entity_ids[1], entity_ids[1]));
        assert!(result.dominates(entity_ids[1], entity_ids[3]));
        assert!(!result.dominates(entity_ids[1], entity_ids[2]));

        // Node 2 dominates {2, 3} (not 1, which is parallel branch)
        assert!(result.dominates(entity_ids[2], entity_ids[2]));
        assert!(result.dominates(entity_ids[2], entity_ids[3]));
    ...
    fn test_dominators_multiple_exits() {
        // Scenario: CFG with multiple exits
        // Expected: Common ancestors dominate all paths
        ...
        // Entry and node 1 should dominate all nodes (common to all paths)
        for &node in &entity_ids {
            assert!(result.dominates(entry, node), "Entry should dominate {}", node);
            assert!(
                result.dominates(entity_ids[1], node),
                "Node 1 should dominate {}",
                node
            );
        }

        // Nodes 2 and 3 don't dominate each other (different exit paths)
        assert!(!result.dominates(entity_ids[2], entity_ids[3]));
        assert!(!result.dominates(entity_ids[3], entity_ids[2]));
    }
}
