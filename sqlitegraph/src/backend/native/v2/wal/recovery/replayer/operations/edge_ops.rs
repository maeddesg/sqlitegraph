...
impl super::DefaultReplayOperations {
    ...
    pub fn handle_edge_insert(
        ...
        debug_log!(
            "Replaying edge insert: cluster_key={:?}, insertion_point={}, edge_len={}",
            cluster_key,
            insertion_point,
            edge_record.edge_data.len()
        );
        ...
        if insertion_point != u32::MAX && insertion_point > 1000 {
            // Reasonable limit to prevent corruption
            return Err(RecoveryError::validation(format!(
                "Insertion point {} exceeds reasonable limit",
                insertion_point
            )));
        }
        ...
        let cluster_data = {
            ...
            .map_err(|e| {
                RecoveryError::replay_failure(format!("Failed to create EdgeCluster: {:?}", e))
            })?;
            ...
            debug_log!(
                "Created cluster data: {} bytes total (edge_count=1)",
                cluster_bytes.len()
            );
            cluster_bytes
        };
        ...
        let allocated_offset = {
            ...
            let mut allocated_offset =
                free_space_manager.allocate(cluster_size_u32).map_err(|e| {
                    RecoveryError::replay_failure(format!(
                        "Failed to allocate space for edge cluster: {:?}",
                        e
                    ))
                })?;
            ...
            let cluster_floor = {
                let graph_file = self.graph_file.read().map_err(|e| {
                    RecoveryError::replay_failure(format!(
                        "Failed to lock graph file for cluster_floor query: {}",
                        e
                    ))
                })?;
            ...
            if allocated_offset < cluster_floor {
                debug_log!(
                    "Allocated offset {} is below cluster floor {}, padding to {}",
                    allocated_offset,
                    cluster_floor,
                    cluster_floor
                );
            ...
            debug_log!(
                "Successfully allocated {} bytes for edge cluster at offset {}",
                cluster_data.len(),
                allocated_offset
            );
            allocated_offset
        ...
        // Step 6: Write cluster data to allocated storage following handle_cluster_create pattern
        {
            ...
            graph_file
                .write_bytes(allocated_offset, &cluster_data)
                .map_err(|e| {
                    RecoveryError::replay_failure(format!(
                        "Failed to write edge cluster to graph file: {:?}",
                        e
                    ))
                })?;

            debug_log!(
                "Successfully wrote edge cluster data: {} bytes to offset {}",
                cluster_data.len(),
                allocated_offset
            );
        ...
        println!(
            "[DEBUG] About to update NodeRecordV2: node_id={}, allocated_offset={}, cluster_data_len={}",
            node_id,
            allocated_offset,
            cluster_data.len()
        );
        {
            ...
            match cluster_direction {
                crate::backend::native::v2::edge_cluster::Direction::Outgoing => {
                    ...
                    debug_log!(
                        "Before write: node_id={}, outgoing_cluster_offset={}, outgoing_edge_count={}",
                        node_record.id,
                ...
                crate::backend::native::v2::edge_cluster::Direction::Incoming => {
                    ...
                    debug_log!(
                        "Before write: node_id={}, incoming_cluster_offset={}, incoming_edge_count={}",
                        node_record.id,
        ...
        self.statistics
            .record_bytes_written(cluster_data.len() as u64);

        debug_log!(
            "Successfully completed edge insert: cluster_key={:?}, insertion_point={}, offset={}, size={}",
            cluster_key,
            insertion_point,
            allocated_offset,
            cluster_data.len()
    ...
    pub fn handle_edge_update(
        ...
        debug_log!(
            "Replaying edge update: cluster_key={:?}, position={}, old_edge_len={}, new_edge_len={}",
            cluster_key,
            position,
            old_edge.edge_data.len(),
            new_edge.edge_data.len()
        );
        ...
        if position > 10000 {
            ...
            return Err(RecoveryError::validation(format!(
                "Position {} exceeds reasonable limit",
                position
            )));
        }
        ...
        let (cluster_offset, cluster_size) = {
            ...
            let node_record = node_store
                .read_node_v2(node_id as crate::backend::native::NativeNodeId)
                .map_err(|e| {
                    RecoveryError::replay_failure(format!(
                        "Failed to read node {} from NodeStore: {}",
                        node_id, e
                    ))
                })?;
            ...
            let (cluster_offset, cluster_size) = match direction {
                crate::backend::native::v2::edge_cluster::Direction::Outgoing => {
                    debug_log!(
                        "Reading Outgoing cluster: offset={}, size={}, node_id={}",
                        node_record.outgoing_cluster_offset,
                    ...
                    if node_record.outgoing_cluster_offset == 0 {
                        return Err(RecoveryError::validation(format!(
                            "Node {} has no outgoing cluster to update",
                            node_id
                        )));
                    }
                    (
                ...
                crate::backend::native::v2::edge_cluster::Direction::Incoming => {
                    debug_log!(
                        "Reading Incoming cluster: offset={}, size={}, node_id={}",
                        node_record.incoming_cluster_offset,
                    ...
                    if node_record.incoming_cluster_offset == 0 {
                        return Err(RecoveryError::validation(format!(
                            "Node {} has no incoming cluster to update",
                            node_id
                        )));
                    }
                    (
        ...
        let mut existing_edges = {
            ...
            EdgeCluster::verify_serialized_layout(&cluster_buffer).map_err(|e| {
                RecoveryError::replay_failure(format!(
                    "Cluster layout verification failed: {:?}",
                    e
                ))
            })?;
        ...
        if position >= existing_edges.len() as u32 {
            return Err(RecoveryError::validation(format!(
                "Position {} out of bounds for cluster with {} edges",
                position,
                existing_edges.len()
            )));
        }
        ...
        if edge_at_position_bytes != old_edge_bytes {
            warn_log!(
                "Edge at position {} differs from expected old_edge - proceeding anyway for data recovery",
                position
            );
        ...
        debug_log!(
            "Updated edge at position {} in cluster for node {:?} direction {:?}",
            position,
            node_id,
            direction
        );
        ...
        let updated_cluster_data = {
            ...
            let updated_cluster =
                EdgeCluster::create_from_compact_edges(existing_edges, node_id, direction)
                    .map_err(|e| {
                        RecoveryError::replay_failure(format!(
                            "Failed to create updated cluster: {:?}",
                            e
                        ))
                    })?;
        ...
        let allocated_offset = {
            ...
            let mut allocated_offset =
                free_space_manager.allocate(cluster_size_u32).map_err(|e| {
                    RecoveryError::replay_failure(format!(
                        "Failed to allocate space for updated edge cluster: {:?}",
                        e
                    ))
                })?;
            ...
            debug_log!(
                "Successfully allocated {} bytes for updated edge cluster at offset {}",
        ...
        // Step 9: Write updated cluster data to allocated storage
        {
            ...
            graph_file
                .write_bytes(allocated_offset, &updated_cluster_data)
                .map_err(|e| {
                    RecoveryError::replay_failure(format!(
                        "Failed to write updated edge cluster: {:?}",
                        e
                    ))
                })?;

            debug_log!(
                "Successfully wrote updated edge cluster: {} bytes to offset {}",
                updated_cluster_data.len(),
                allocated_offset
            );
        }; // GraphFile lock is released here
        ...
        // Step 10: Update NodeRecordV2 with new cluster offset
        {
            ...
            let mut node_record = node_store
                .read_node_v2(node_id as crate::backend::native::NativeNodeId)
                .map_err(|e| {
                    RecoveryError::replay_failure(format!(
                        "Failed to read NodeRecordV2 for update: {}",
                        e
                    ))
                })?;
            ...
            debug_log!(
                "Before NodeRecordV2 update: node_id={}, edge_count={}, allocated_offset={}, cluster_size={}",
                node_record.id,
            ...
            debug_log!(
                "After NodeRecordV2 update: node_id={}, outgoing_edge_count={}, outgoing_offset={}, outgoing_size={}",
                node_record.id,
                node_record.outgoing_edge_count,
                node_record.outgoing_cluster_offset,
                node_record.outgoing_cluster_size
            ...
            // Write updated NodeRecordV2 back
            node_store.write_node_v2(&node_record).map_err(|e| {
                RecoveryError::replay_failure(format!("Failed to update NodeRecordV2: {:?}", e))
            })?;

            debug_log!(
                "Updated NodeRecordV2 cluster reference for node {:?} direction {:?} to offset {}",
                node_id,
                direction,
                allocated_offset
            );
        ...
        self.statistics.record_edge_operation();
        self.statistics
            .record_bytes_written(updated_cluster_data.len() as u64);

        debug_log!(
            "Successfully completed edge update: cluster_key={:?}, position={}, old_offset={}, new_offset={}, old_size={}, new_size={}",
            cluster_key,
            position,
            cluster_offset,
            allocated_offset,
    ...
    pub fn handle_edge_delete(
        ...
        let (cluster_offset, cluster_size) = {
            ...
            let (cluster_offset, cluster_size) = match direction {
                crate::backend::native::v2::edge_cluster::Direction::Outgoing => {
                    if node_record.outgoing_cluster_offset == 0 {
                        return Err(RecoveryError::validation(format!(
                            "Node {} has no outgoing cluster to delete from",
                            node_id
                        )));
                    }
                    (
                ...
                crate::backend::native::v2::edge_cluster::Direction::Incoming => {
                    if node_record.incoming_cluster_offset == 0 {
                        return Err(RecoveryError::validation(format!(
                            "Node {} has no incoming cluster to delete from",
                            node_id
                        )));
                    }
                    (
        ...
        debug_log!(
            "Deleted edge at position {} in cluster for node {:?} direction {:?} - {} edges remaining",
            position,
            node_id,
            direction,
            existing_edges.len()
        ...
        let updated_cluster_data = {
            ...
            if existing_edges.is_empty() {
                debug_log!(
                    "Cluster became empty after deleting last edge for node {:?} direction {:?}",
                    node_id,
                    direction
                );
                ...
                .map_err(|e| {
                    RecoveryError::replay_failure(format!(
                        "Failed to create empty cluster: {:?}",
                        e
                    ))
                })?;
                ...
                let updated_cluster =
                    EdgeCluster::create_from_compact_edges(existing_edges, node_id, direction)
                        .map_err(|e| {
                            RecoveryError::replay_failure(format!(
                                "Failed to create updated cluster after edge deletion: {:?}",
                                e
                            ))
                        })?;
        ...
        let allocated_offset = {
            ...
            let cluster_size_u32 = updated_cluster_data.len() as u32;
            let allocated_offset = free_space_manager.allocate(cluster_size_u32).map_err(|e| {
                RecoveryError::replay_failure(format!(
                    "Failed to allocate space for updated edge cluster: {:?}",
                    e
                ))
        ...
        debug_log!(
            "Successfully completed edge delete: cluster_key={:?}, position={}, old_offset={}, new_offset={}, old_size={}, new_size={}",
            cluster_key,
            position,
            cluster_offset,
            if updated_cluster_data.len() == 0 {
