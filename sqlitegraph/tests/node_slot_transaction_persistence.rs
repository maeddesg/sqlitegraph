...
fn test_node_slots_persist_across_edge_transactions() {
    ...
    let mut node_ids = Vec::new();
    {
        ...
        for i in 1..=300 {
            ...
            if i % 50 == 0 {
                ...
                assert_eq!(version, 2, "Node {} should have version=2, got {}", node_id, version);
    ...
    // Phase 2: Reopen and verify all node slots still have version=2
    {
        ...
        for &node_id in &[256i64, 257i64, 258i64] {
            let version = read_node_slot_version(&db_path, node_id)
                .expect(&format!("Failed to read version for critical node {}", node_id));
            assert_eq!(version, 2, "Critical node {} should have version=2 after reopen, got {}", node_id, version);
            println!("Critical node {} has version={} after reopen", node_id, version);
        }
        ...
        for &node_id in [1, 50, 100, 150, 200, 250, 300].iter() {
            if node_id <= 300 {
                let version = read_node_slot_version(&db_path, node_id)
                    .expect(&format!("Failed to read version for sample node {}", node_id));
                assert_eq!(version, 2, "Sample node {} should have version=2 after reopen, got {}", node_id, version);
                ...
                assert_eq!(node.name, format!("node_{}", node_id), "Node name corrupted after reopen");
            }
        }

        println!("✅ All critical and sample nodes verified with version=2 after transaction/reopen");
    }
}
...
fn test_file_size_never_shrinks_during_edge_operations() {
    ...
    // Phase 2: Create many edges (triggers internal transactions)
    {
        ...
        for i in 0..2000 {
            ...
            if i % 500 == 0 {
                let current_size = std::fs::metadata(&db_path).unwrap().len();
                assert!(current_size >= max_file_size,
                    "File size shrunk during edge operations: max was {}, now {}", max_file_size, current_size);
                max_file_size = max_file_size.max(current_size);
                println!("After {} edges, file size: {} bytes", i, current_size);
            }
        }

        println!("✅ Created 2000 edges, file size never shrunk below max: {}", max_file_size);
        drop(graph);
    }
    ...
    // Phase 3: Reopen and verify all nodes still exist with version=2
    {
        ...
        for &node_id in &[1, 50, 100, 150, 200] {
            let version = read_node_slot_version(&db_path, node_id)
                .expect(&format!("Failed to read version for node {} after edge operations", node_id));
            assert_eq!(version, 2, "Node {} should have version=2 after edge operations, got {}", node_id, version);
        }
        ...
        assert!(final_size >= max_file_size,
            "Final file size {} is smaller than max size {}", final_size, max_file_size);

        println!("✅ All {} nodes verified after edge operations, final file size: {}", node_ids.len(), final_size);
    }
}