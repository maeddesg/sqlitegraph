...
fn create_test_graph() -> NativeResult<(TempDir, GraphFile)> {
    ...
    let graph_file = GraphFile::create_or_open(&db_path)?;
...
fn test_delete_isolated_node() {
    ...
    // Verify node count was decremented
    let node_count_after = graph_file.header().node_count;
    assert_eq!(
        node_count_after,
        node_count_before - 1,
        "Node count should be decremented by 1"
    );
}
...
fn test_delete_node_with_edges() {
    ...
    // Delete node 2
    let result = node_store.delete_node_with_edges(2);
...
fn test_deletion_updates_header() {
    ...
    let node_count_after = graph_file.header().node_count;
    assert_eq!(node_count_after, 3, "Should have 3 nodes after deleting 2");
}
...
fn test_delete_nonexistent_node() {
    ...
    let result = node_store.delete_node_with_edges(999);
...
fn test_delete_node_clears_cluster_metadata() {
    ...
    let original_outgoing_count = node_before.outgoing_edge_count;
    let original_incoming_count = node_before.incoming_edge_count;

    // Delete node 1
    let result = node_store.delete_node_with_edges(1);
