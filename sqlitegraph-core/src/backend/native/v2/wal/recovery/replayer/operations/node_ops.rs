...
impl super::DefaultReplayOperations {
    ...
    pub fn handle_node_delete(
        ...
        // Create NodeStore and FreeSpaceManager for this operation following proper SME methodology
        {
            ...
            // Use delete_node_with_offset with explicit slot_offset from WAL record
            // to skip the node existence check (needed for recovery scenarios)
            node_store
                .delete_node_with_offset(node_id as NativeNodeId, Some(slot_offset))
                .map_err(|e| {
                    RecoveryError::io_error(format!(
