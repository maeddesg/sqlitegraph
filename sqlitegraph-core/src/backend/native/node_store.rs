...
impl<'a> NodeStore<'a> {
    ...
    pub fn read_node_v2(&mut self, node_id: NativeNodeId) -> NativeResult<NodeRecordV2> {
        ...
        // Check if slot is zeroed (deleted node)
        // A deleted slot has all zeros in the first bytes (no V2 magic)
        if header_buffer[0] == 0 && header_buffer[1] == 0 {
            // Check if entire header prefix is zero (deleted slot)
            let is_zeroed = header_buffer.iter().all(|&b| b == 0);
            if is_zeroed {
                return Err(NativeBackendError::CorruptNodeRecord {
                    node_id,
                    reason: "Node slot is empty (node was deleted)".to_string(),
                });
            }
        }
    ...
    ///
    /// This implementation:
    /// 1. Validates the node exists (unless explicit slot_offset provided for recovery)
    /// 2. Zeroes out the 4096-byte node slot on disk
    /// 3. Removes from node_index
    ///
    /// NOTE: node_count is NOT decremented because node IDs are fixed slot-based.
    /// Decrementing node_count would make higher-numbered nodes unreadable.
    ///
    /// NOTE: Edge cluster cleanup is intentionally NOT handled here - it will be
    /// implemented as a separate phase to properly handle orphaned clusters.
    pub fn delete_node(&mut self, node_id: NativeNodeId) -> NativeResult<()> {
        self.delete_node_with_offset(node_id, None)
    }

    /// Delete a node record by ID with optional explicit slot offset
    ///
    /// This variant is used during WAL recovery where the slot_offset is known
    /// from the WAL record. When provided, it skips the node existence check
    /// and uses the explicit offset.
    ///
    /// # Arguments
    /// * `node_id` - The ID of the node to delete
    /// * `slot_offset` - Optional explicit file offset (for recovery scenarios)
    pub fn delete_node_with_offset(
        &mut self,
        ...
        let slot_offset = match slot_offset {
            Some(offset) => offset,
            None => {
                // Validate node exists by attempting to read it
                // read_node_v2 returns InvalidNodeId error if node doesn't exist
                let _record = self.read_node_v2(node_id)?;
                let node_data_offset = self.graph_file.persistent_header().node_data_offset;
                node_data_offset + ((node_id - 1) as u64 * constants::node::NODE_SLOT_SIZE)
            }
        };

        // Zero out the 4096-byte node slot on disk
        let zero_buffer = vec![0u8; constants::node::NODE_SLOT_SIZE as usize];

        #[cfg(feature = "v2_experimental")]
        {
            self.graph_file
                .mmap_write_bytes(slot_offset, &zero_buffer)?;
        }
        ...
        // DO NOT decrement node_count - node IDs are fixed slot addresses
        // node_count represents the highest allocated slot, not active nodes.
        // Decrementing it would break read_node_v2 for nodes with IDs > new count.
