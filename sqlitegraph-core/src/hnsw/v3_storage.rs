...
use crate::hnsw::storage::{VectorStorage, VectorBatch, VectorRecord};
...
impl VectorStorage for V3VectorStorageHandle {
    fn store_vector(&mut self, vector: &[f32], metadata: Option<Value>) -> Result<u64, HnswError> {
        ...
        let json_value = serde_json::to_value(&stored).map_err(|e| {
            HnswError::Storage(HnswStorageError::IoError(format!("Serialization error: {}", e)))
        })?;
        
        let key = self.vector_key(id);
        unsafe {
            self.backend().kv_set_v3(key, KvValue::Json(json_value), None);
        }
        ...
        match value {
            Some(KvValue::Json(json)) => {
                let stored: StoredVectorRecord = serde_json::from_value(json).map_err(|e| {
                    HnswError::Storage(HnswStorageError::IoError(format!("Deserialization error: {}", e)))
                })?;
                Ok(Some(stored.data))
            }
...
impl V3Backend {
    ...
    /// ```
    pub fn create_hnsw_storage(&self, index_name: impl Into<String>) -> Option<Box<dyn VectorStorage>> {
