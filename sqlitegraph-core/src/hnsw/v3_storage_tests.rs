...
fn test_v3_storage_store_with_explicit_id() {
    ...
    storage.store_vector_with_id(explicit_id, vector.clone(), None).unwrap();
...
fn test_v3_storage_list_vectors() {
    ...
    assert!(listed.is_empty(), "list_vectors returns empty due to prefix scan limitation");
...
fn test_v3_storage_with_graph_operations() {
    ...
    use crate::backend::{NodeSpec, EdgeSpec, GraphBackend};
