...
use sqlitegraph::backend::{GraphBackend, NodeSpec, EdgeSpec, SqliteGraphBackend};
use sqlitegraph::backend::native::v3::V3Backend;
use sqlitegraph::algo::backend::{bfs_traversal, dfs_traversal, k_hop_neighbors, shortest_path, bfs};
...
fn main() {
    ...
    for (graph_name, node_count, edge_count) in test_graphs {
        println!("\n{}", "=".repeat(70));
        println!("Graph: {} ({} nodes, {} edges)", graph_name, node_count, edge_count);
        println!("{}", "=".repeat(70));
...
fn populate_v3_backend(backend: &V3Backend, node_count: usize, edge_count: usize) {
    ...
    for i in 0..node_count {
        batch.insert_node(sqlitegraph::backend::NodeSpec {
            kind: "Node".to_string(),
            name: format!("node_{}", i),
            file_path: None,
            data: serde_json::json!({"id": i}),
    ...
    for i in 0..edge_count {
        ...
        if from != to {
            batch.insert_edge(sqlitegraph::backend::EdgeSpec {
                from,
                to,
                edge_type: "Edge".to_string(),
                data: serde_json::json!({}),
    ...
    F: Fn(&dyn GraphBackend) -> Result<(), String>,
{
    ...
    println!("  {:30} | SQLite: {:8.2}ms | V3: {:8.2}ms | {:6.2}x {}", 
             name, sqlite_ms, v3_ms, speedup,
             if sqlite_result.is_ok() && v3_result.is_ok() { "✓" } else { "✗" });
...
fn benchmark_k_hop(nodes: usize, edges: usize) -> Vec<BenchmarkResult> {
    benchmark_both("k_hop (depth=3)", nodes, edges, |backend| {
        ...
        let result = backend.k_hop(snapshot, 1, 3, sqlitegraph::backend::BackendDirection::Outgoing)
...
fn benchmark_node_degree(nodes: usize, _edges: usize) -> Vec<BenchmarkResult> {
    benchmark_both("node_degree", nodes, 0, |backend| {
        let snapshot = SnapshotId::current();
        let _degree = backend.node_degree(snapshot, 1).map_err(|e| e.to_string())?;
        Ok(())
    })
}
...
fn benchmark_insert_node(nodes: usize) -> Vec<BenchmarkResult> {
    ...
    let sqlite_result: Result<(), String> = (|| {
        for i in 0..100 {
            sqlite_backend.insert_node(NodeSpec {
                kind: "Test".to_string(),
                name: format!("test_{}", i),
                file_path: None,
                data: serde_json::json!({}),
    ...
    println!("  {:30} | SQLite: {:8.2}ms | V3: {:8.2}ms | {:6.2}x {}", 
             "insert_node (100)", sqlite_ms, v3_ms, speedup,
             if sqlite_result.is_ok() && v3_result.is_ok() { "✓" } else { "✗" });
...
fn benchmark_insert_edge(nodes: usize) -> Vec<BenchmarkResult> {
    ...
    let sqlite_result: Result<(), String> = (|| {
        for i in 0..100 {
            ...
            sqlite_backend.insert_edge(EdgeSpec {
                from,
                to,
    ...
    println!("  {:30} | SQLite: {:8.2}ms | V3: {:8.2}ms | {:6.2}x {}", 
             "insert_edge (100)", sqlite_ms, v3_ms, speedup,
             if sqlite_result.is_ok() && v3_result.is_ok() { "✓" } else { "✗" });
...
fn benchmark_neighbors_outgoing(nodes: usize, edges: usize) -> Vec<BenchmarkResult> {
    benchmark_both("neighbors (outgoing)", nodes, edges, |backend| {
        ...
        let _neighbors = backend.neighbors(snapshot, 1, query).map_err(|e| e.to_string())?;
        Ok(())
    })
}
...
fn benchmark_neighbors_both(nodes: usize, edges: usize) -> Vec<BenchmarkResult> {
    benchmark_both("neighbors (undirected)", nodes, edges, |backend| {
        ...
        let _outgoing = backend.neighbors(snapshot, 1, query_out).map_err(|e| e.to_string())?;
        let _incoming = backend.neighbors(snapshot, 1, query_in).map_err(|e| e.to_string())?;
        Ok(())
    })
}
...
fn print_summary(results: &[BenchmarkResult]) {
    println!("\n\n");
    println!("╔══════════════════════════════════════════════════════════════════════════════════════════════════╗");
    println!("║                         ALGORITHM BENCHMARK SUMMARY ({} Tests)", results.len());
    println!("╚══════════════════════════════════════════════════════════════════════════════════════════════════╝");
    
    // Group by graph size
    let mut by_size: std::collections::HashMap<(usize, usize), Vec<&BenchmarkResult>> = std::collections::HashMap::new();
    for r in results {
        by_size.entry((r.graph_nodes, r.graph_edges)).or_default().push(r);
    }
    ...
    for ((nodes, edges), group) in by_size {
        ...
        for algo in algo_names {
            let sqlite = group.iter().find(|r| r.algorithm == algo && r.backend == "SQLite");
            let v3 = group.iter().find(|r| r.algorithm == algo && r.backend == "V3");
            
            if let (Some(s), Some(v)) = (sqlite, v3) {
                let speedup = if v.elapsed_ms > 0.0 { s.elapsed_ms / v.elapsed_ms } else { 0.0 };
    ...
    let sqlite_tests = results.iter().filter(|r| r.backend == "SQLite" && r.success).count();
    let v3_tests = results.iter().filter(|r| r.backend == "V3" && r.success).count();
    
    println!("\n{}", "=".repeat(90));
    println!("TOTAL: {} tests | Passed: {} | Failed: {}", total_tests, passed_tests, total_tests - passed_tests);
    println!("SQLite: {}/{} passed | V3: {}/{} passed", 
             sqlite_tests, total_tests/2, v3_tests, total_tests/2);
    ...
    for (_, group) in grouped {
        if let (Some(s), Some(v)) = (group.iter().find(|r| r.backend == "SQLite"),
    ...
    if !speedups.is_empty() {
        ...
        println!("Average Speedup: {:.2}x | Max Speedup: {:.2}x", avg_speedup, max_speedup);
    }
}
