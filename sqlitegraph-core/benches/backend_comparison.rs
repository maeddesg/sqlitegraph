...
impl BenchmarkEnvironment {
    pub fn detect() -> Self {
        Self {
            ...
            rust_version: rustc_version::version().map(|v| v.to_string()).unwrap_or_default(),
            rustc_host: std::env::consts::ARCH.to_string(),
            cargo_profile: if cfg!(debug_assertions) { "debug".to_string() } else { "release".to_string() },
        ...
        group.bench_with_input(
            ...
            |b, (data, k)| {
                b.iter_batched(
                    ...
                    |backend| {
                        let result = k_hop_neighbors(&backend, black_box(1), black_box(*k)).unwrap();
                        black_box(result.len());
                    },
                    criterion::BatchSize::SmallInput,
                );
        ...
        group.bench_with_input(
            ...
            |b, (data, target)| {
                b.iter_batched(
                    ...
                    |backend| {
                        let result = shortest_path(&backend, black_box(1), black_box(*target)).unwrap();
